...
NOTE:  The ONTAP CLI must be used for adding additional SVMs to an HA CVO working environment.  The ONTAP System Manager interface does not support the required "probe-port" parameter which must be set on LIFs in an HA CVO working environment.  Using System Manager will create unsupported LIFs which will not failover correctly during an HA takeover event and distrupt client access to the storage.
