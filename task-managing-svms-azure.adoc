...
* One IP address for NAS data LIF access from node 1
* One IP address for NAS data LIF access from node 2
* One IP address for an iSCSI LIF on node 1 VM NIC0
* One IP address for an iSCSI LIF on node 2 VM NIC0
+
...
+
NOTE: A frontend iSCSI IP address should only be created on VM NIC0 for each corresponding SVM being added. Do not create a frontend IP address on the Azure loadbalancer. For more information on how to add the IP address to the VM NIC0, see "Step 7 [hyperlink]"
