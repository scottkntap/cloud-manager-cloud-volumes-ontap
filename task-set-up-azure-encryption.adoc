...
Cloud Volumes ONTAP data is automatically encrypted in Azure using https://azure.microsoft.com/en-us/documentation/articles/storage-service-encryption/[Azure Storage Service Encryption^]. The default implementation uses a Microsoft-managed key. No setup is required. When using a customer-managed key, all managed disks and storage accounts deployed for Cloud Volumes ONTAP including those used for tiering are encrypted using the same customer-managed key.
